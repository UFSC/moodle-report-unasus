...
class report_unasus_renderer extends plugin_renderer_base {
    ...
    public function filter_modulo_tutor_polo(){
        ...
        $output .= html_writer::select(array('Em Aberto', 'Em Dia', 'Expirado', 'Fora do Prazo'), 'prazo_select', '', false);

        $output .= html_writer::start_tag('div');

        $output .= html_writer::start_tag('div', array('class'=>'relatorio-unasus multiple_list'));
        $output .= html_writer::nonempty_tag('p', 'Modulos');
        $output .= html_writer::select(get_nomes_modulos(), 'multiple_modulo', '', false, array('multiple'=>'multiple'));
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_tag('div', array('class'=>'relatorio-unasus multiple_list'));
        $output .= html_writer::nonempty_tag('p', 'Polos');
        $output .= html_writer::select(get_nomes_polos(), 'multiple_polo', '', false, array('multiple'=>'multiple'));
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_tag('div', array('class'=>'relatorio-unasus multiple_list'));
        $output .= html_writer::nonempty_tag('p', 'Tutores');
        $output .= html_writer::select(get_nomes_tutores(), 'multiple_tutor', '', false, array('multiple'=>'multiple'));
        $output .= html_writer::end_tag('div');
...
//        $mform->addElement('hidden', 'relatorio', $this-
//        >_customdata['relatorio']);
//
